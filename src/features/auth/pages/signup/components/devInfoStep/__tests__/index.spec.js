...
describe('DevInfoStep', () => {
    ...
    it('deve renderizar todos os campos com labels corretos', () => {
        render(_jsx(DevInfoStep, { onChange: mockOnChange, data: defaultData }));
    ...
    it('deve exibir os valores iniciais corretamente', () => {
        render(_jsx(DevInfoStep, { onChange: mockOnChange, data: defaultData }));
        expect(screen.getByLabelText(/Link do Github/i)).toHaveValue('https://github.com/gustavo');
        expect(screen.getByLabelText(/Link do Linkedin/i)).toHaveValue('https://linkedin.com/in/gustavo');
        expect(screen.getByLabelText(/Bio/i)).toHaveValue('Desenvolvedor fullstack');
        // stack vira string unida por ", "
        expect(screen.getByLabelText(/Stack/i)).toHaveValue('React, Node.js, PostgreSQL');
    });
    it('deve chamar onChange ao digitar nos campos', () => {
        ...
        expect(mockOnChange).toHaveBeenCalledWith('githubLink', 'https://github.com/akira');
        fireEvent.change(screen.getByLabelText(/Link do Linkedin/i), {
            target: { value: 'https://linkedin.com/in/akira' },
        });
        expect(mockOnChange).toHaveBeenCalledWith('linkedinLink', 'https://linkedin.com/in/akira');
